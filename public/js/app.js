...
(function () {
  ...
  const AUTH_REQUIRED_PAGES = ['chat', 'history', 'settings', 'dashboard'];
  const APP_PAGES = ['chat', 'history', 'settings', 'pricing', 'dashboard'];
  const TITLES = { chat: 'AI Chat', history: 'Riwayat', settings: 'Pengaturan', pricing: 'Harga', dashboard: 'Dashboard' };
  ...
  function applyAuthUI() {
    ...
    if (on) {
      ...
      $('.user-chip__meta span').textContent = state.user.plan === 'free' ? 'Paket Free' : state.user.plan;
    }
  ...
  async function boot() {
    ...
    try { [session, modelsRes] = await Promise.all([apiGet('/api/auth/session'), apiGet('/api/models')]); }
