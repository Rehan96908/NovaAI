...
(function () {
  ...
  function renderSettings() {
    ...
    const kInp = document.getElementById('stApiKey'); if (kInp) kInp.value = u.settings.apiKey || '';
  ...
  const kInp2 = document.getElementById('stApiKey'); if (kInp2) kInp2.addEventListener('change', (e) => saveSettings({ apiKey: e.target.value.trim() }));
  ...
  function renderAdminMaint() {
    ...
    inp.value = m.message || '';
    const setBadge = document.getElementById('settingsMaintBadge');
    const setInp = document.getElementById('settingsMaintMsg');
    const setBtn = document.getElementById('btnSettingsToggleMaint');
    if (setBadge && setBtn && setInp) {
      if (m.enabled) {
        setBadge.className = 'badge badge--disabled';
        setBadge.innerHTML = icon('bell', 'icon--sm') + ' Maintenance AKTIF';
        setBtn.textContent = 'Matikan Server Maintenance';
        setBtn.className = 'btn btn--outline';
      } else {
        setBadge.className = 'badge badge--active';
        setBadge.innerHTML = icon('check', 'icon--sm') + ' Server Normal (Online)';
        setBtn.textContent = 'Aktifkan Server Maintenance';
        setBtn.className = 'btn btn--primary';
      }
      setInp.value = m.message || '';
    }
  }
