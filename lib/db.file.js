...
function flush() {
  ...
  try {
    ...
    console.error('[db] gagal menyimpan:', e.message);
  }
}
function save() {
  dirty = true;
  if (timer) return;
  timer = setTimeout(() => { timer = null; try { flush(); } catch (e) { console.error('[db] gagal menyimpan:', e.message); } }, 150);
}
...
async function flushIfDirty() { if (dirty || timer) flush(); }
const mode = 'file';
