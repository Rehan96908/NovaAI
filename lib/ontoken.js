...
function friendly(status, text) {
  ...
  if (/cooldown/i.test(detail)) return detail;
  if (status === 401 || status === 403) {
    if (/revoked/i.test(detail)) {
      return 'API key ON Token ditolak: kunci berstatus "revoked" (dicabut). Gunakan kunci API aktif di menu Pengaturan -> Model & Respons.';
    }
    return 'API key ON Token ditolak. Anda dapat memasukkan API Key aktif pribadi di menu Pengaturan -> Model & Respons.';
  }
...
async function streamChat({ model, messages, temperature, maxTokens, signal, apiKey, onDelta }) {
  ...
  try {
    ...
    if ((err.status === 401 || err.status === 403 || err.status === 400 || /cooldown/i.test(err.message) || !keyToUse) && process.env.GEMINI_API_KEY && !process.env.MODELS_FILE && process.env.NODE_ENV !== "test") {
